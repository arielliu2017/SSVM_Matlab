...
%setting up weights for C

ratio_plus = sum(labels > 0)/length(labels);

ratio_minus = sum(labels < 0)/length(labels);

assert(abs(ratio_plus+ratio_minus-1)<eps,'ratio computation mistake\n')

C_weights = zeros(size(eP));

C_weights(labels>0) = ratio_plus;

C_weights(labels<0) = ratio_minus;
